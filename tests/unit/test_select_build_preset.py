...
def test_cheapest_preset_is_selected(mock_client):
...
def test_user_selection_is_respected(mock_client):
    mock_client.presets.update(FAKE_PRESETS)
    selected_preset = select_build_preset(
...
def test_when_nothing_fits_first_preset_is_used(mock_client):
    presets = {
        "bad": Preset(cpu=1, memory_mb=9999, credits_per_hour=Decimal("5")),
